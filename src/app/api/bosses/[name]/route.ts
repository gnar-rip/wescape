...
const CACHE_DIR =
  process.env.BOSS_ICON_CACHE_DIR ||
  path.join(process.cwd(), "public", "assets", "bosses");
...
export async function GET(
  ...
  const { name } = params;
  const fileName = bossIcons[name.toLowerCase()];
  if (!fileName) {
  ...
  const upstream = `https://static.runelite.net/cache/boss/${fileName}`;
